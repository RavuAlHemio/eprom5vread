...
pub type R = crate::R<DMA_CMAR3_SPEC>;
#[doc = "Register `DMA_CMAR3` writer"]
pub type W = crate::W<DMA_CMAR3_SPEC>;
...
are ignored. Access is automatically aligned to a word address. In memory-to-memory mode, this register identifies the memory source address if DIR=1 and the memory destination address if DIR=0. In peripheral-to-peripheral mode, this register identifies the peripheral source address DIR=1 and the peripheral destination address if DIR=0. Note: this register is set and cleared by software. It must not be written when the channel is enabled (EN = 1). It is not read-only when the channel is enabled (EN=1)."]
    #[inline(always)]
    #[must_use]
    pub fn ma(&mut self) -> MA_W<DMA_CMAR3_SPEC> {
        MA_W::new(self, 0)
    }
...
pub struct DMA_CMAR3_SPEC;
impl crate::RegisterSpec for DMA_CMAR3_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [`dma_cmar3::R`](R) reader structure"]
impl crate::Readable for DMA_CMAR3_SPEC {}
#[doc = "`write(|w| ..)` method takes [`dma_cmar3::W`](W) writer structure"]
impl crate::Writable for DMA_CMAR3_SPEC {
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
#[doc = "`reset()` method sets DMA_CMAR3 to value 0"]
impl crate::Resettable for DMA_CMAR3_SPEC {
